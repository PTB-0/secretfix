...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'commitguard-config-'));
});
...
describe('config', () => {
  ...
  it('merges partial config over defaults', () => {
    writeFileSync(join(dir, '.commitguardrc.json'), JSON.stringify({ owasp: false }));
  ...
  it('falls back to defaults when the config file is malformed', () => {
    writeFileSync(join(dir, '.commitguardrc.json'), '{ not valid json');
    expect(loadConfig(dir)).toEqual(defaultConfig);
  });

  it('ignores non-boolean values in the config file', () => {
    writeFileSync(join(dir, '.commitguardrc.json'), JSON.stringify({ secrets: 'nope', ignoreLines: 'nope' }));
    const config = loadConfig(dir);
    expect(config.secrets).toBe(true);
...
describe('isExcluded', () => {
  ...
  it('keeps the built-in excludes when the user adds their own', () => {
    writeFileSync(join(dir, '.commitguardrc.json'), JSON.stringify({ excludeFiles: ['test/fixtures/'] }));
    const config = loadConfig(dir);
...
describe('hasIgnoreMarker', () => {
  it('suppresses a finding when the previous line carries the marker', () => {
    const content = ['// commitguard-ignore-next-line', 'const key = "AKIAABCDEFGHIJKLMNOP";'].join('\n');
    expect(hasIgnoreMarker(content, 2)).toBe(true);
  });

  it('suppresses a finding when the marker is a trailing comment on the same line', () => {
    const content = 'const key = "AKIAABCDEFGHIJKLMNOP"; // commitguard-ignore\n';
  ...
  it('does not suppress a line two below the marker', () => {
    const content = ['// commitguard-ignore-next-line', 'const a = 1;', 'const key = "AKIAABCDEFGHIJKLMNOP";'].join('\n');
