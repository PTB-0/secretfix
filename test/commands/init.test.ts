...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'secretfix-init-'));
});
...
describe('initCommand', () => {
  ...
  it('writes the pre-commit hook running secretfix scan', () => {
    initCommand(dir, { installHusky: vi.fn() });
    const hook = readFileSync(join(dir, '.husky', 'pre-commit'), 'utf8');
    expect(hook).toContain('npx secretfix scan');
  });

  it('writes a default .secretfixrc.json if none exists', () => {
    initCommand(dir, { installHusky: vi.fn() });
    const config = JSON.parse(readFileSync(join(dir, '.secretfixrc.json'), 'utf8'));
    expect(config.secrets).toBe(true);
  });

  it('does not overwrite an existing .secretfixrc.json', () => {
    mkdirSync(join(dir, '.husky'), { recursive: true });
    writeFileSync(join(dir, '.secretfixrc.json'), JSON.stringify({ secrets: false }));
  ...
  it('is idempotent — a second run does not add the invocation twice', () => {
    ...
    const hook = readFileSync(join(dir, '.husky', 'pre-commit'), 'utf8');
    expect(hook.match(/npx secretfix scan/g)).toHaveLength(1);
  });
  ...
  it('falls back to .git/hooks/pre-commit when husky cannot be installed', () => {
    ...
    expect(readFileSync(join(dir, '.git', 'hooks', 'pre-commit'), 'utf8')).toContain('npx secretfix scan');
  });
