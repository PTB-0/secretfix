...
describe('vibeguard init (e2e)', () => {
  it('installs a working hook and a default config', () => {
    ...
    expect(readFileSync(hookPath, 'utf8')).toContain('npx vibeguard scan');
    expect(JSON.parse(readFileSync(join(repoDir, '.vibeguardrc.json'), 'utf8')).secrets).toBe(true);
  });
});
