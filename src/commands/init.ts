...
const HOOK_INVOCATION = 'npx secretfix scan';
 ...
 * — which would make the hook abort before secretfix ever ran, blocking every
...
export function buildHookScript(invocation: string): string {
  ...
  secretfix_stdin=/dev/tty
else
  secretfix_stdin=/dev/null
fi
${invocation} < "$secretfix_stdin"
`;
}

const HOOK_CONTENT = `#!/usr/bin/env sh
# Installed by \`secretfix init\`. Delete this file to remove the hook.
...
export function initCommand(cwd: string, deps: InitDeps = { installHusky: defaultInstallHusky }): void {
  ...
  if (!existsSync(huskyDir)) {
    try {
      ...
      if (!existsSync(join(cwd, '.git'))) {
        throw new Error('secretfix init: not a git repository, and husky could not be installed. Run "git init" first.');
      }
      hookDir = gitHooksDir;
      console.warn('secretfix: husky is unavailable — installing a plain .git/hooks/pre-commit hook instead.');
    }
  }
  ...
  if (!existsSync(configPath)) {
    const template = {
      ...
      ignoreLines: {},
      // Added to secretfix's built-in exclude list (lockfiles, minified bundles).
      excludeFiles: []
    };
  ...
  const message =
    outcome === 'unchanged'
      ? `secretfix: pre-commit hook already present at ${relativeHook}`
      : `secretfix: pre-commit hook ${outcome === 'appended' ? 'appended to' : 'installed at'} ${relativeHook}`;
  console.log(message);
}
